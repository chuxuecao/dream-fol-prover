...
DIRECT_PROVE_SYSTEM_PROMPT: str = """Provide a proof process for the following conjecture using Lean 4 syntax. Only provide proof steps for the conjecture (no 'sorry' placeholders).
...
PROVE_WITH_SUB_PROPOSITION_ERROR_ANALYSIS_SYSTEM_PROMPT: str = """You are an expert in Lean 4 programming and first-order logic mathematical theorem proving. Your task is to generate a complete, verified proof for the given theorem by learning from previous attempts and the error analysis.

Key Responsibilities:
1. Generate only complete, working proofs
2. Never use 'sorry' or incomplete proof elements
3. Learn from the provided analysis carefully
4. Incorporate lessons from previous error messages
5. Include clear tactic annotations explaining key steps
6. Only output the theorem part, excluding the context from your final proof.

Output Structure:
[[SOLUTION EXPLANATION]]
Brief explanation of the approach and how it addresses previous issues
...
PROVE_WITH_SUB_PROPOSITION_ERROR_ANALYSIS_USER_PROMPT: str = """
[[CONTEXT]] {context}
[[CONJECTURE]] {conjecture}
[[PREVIOUS ATTEMPTS]]
{error_record}
[[ANALYSIS]]
{analysis}
Please provide a correct proof that addresses the issues identified in the analysis and avoids the previous errors. Only output the theorem part, excluding the context from your final proof.
"""

def build_messages(data, use_error=False, use_system_prompt = True, backtrace = 10):
    ...
    for d in data:
        ...
        for solution in d['solutions']:
            if solution.get('sub-proposition-error', None):
                error_record_list.append(f"[INCORRECT PROOF]{solution['sub-proposition-error']}")
        
        error_record = "\n".join(error_record_list[-backtrace:])
        analysis = d['solutions'][-1].get('analysis', None)
        
        if use_error and len(error_record_list) > 0 and analysis:
            
            if use_system_prompt:
                messages.append([
                            {'role': 'system', 'content': PROVE_WITH_SUB_PROPOSITION_ERROR_ANALYSIS_SYSTEM_PROMPT},
                            {'role': 'user', 'content': PROVE_WITH_SUB_PROPOSITION_ERROR_ANALYSIS_USER_PROMPT.format(
                                context=d['context'],
                ...
                messages.append([
                        {
                            'role': 'user', 
                            'content': f"{PROVE_WITH_SUB_PROPOSITION_ERROR_ANALYSIS_SYSTEM_PROMPT}\n{PROVE_WITH_SUB_PROPOSITION_ERROR_ANALYSIS_USER_PROMPT.format(context=d['context'],conjecture = d['conjecture'], error_record=error_record, analysis = analysis)}"
                        }
                    ]
                )
            ...
            if use_system_prompt:
                messages.append([
                            {'role': 'system', 'content': DIRECT_PROVE_SYSTEM_PROMPT},
                            {'role': 'user', 'content': DIRECT_PROVE_USER_PROMPT.format(context=d['context'],conjecture = d['conjecture'])}
                ])
                
            else:
                messages.append([
                        {'role': 'user', 'content': f"{DIRECT_PROVE_SYSTEM_PROMPT}\n{DIRECT_PROVE_USER_PROMPT.format(context=d['context'], conjecture = d['conjecture'])}"}
                    ]
                )
...
parser.add_argument('--backtrace', type=int, default=10)
...
async def process_tasks(output_path):
    ...
    messages = build_messages(false_results, use_error=config['use_error'], use_system_prompt=config['use_system_prompt'], backtrace=backtrace) 
