...
SUBGOAL_ERROR_ANALYZE_SYSTEM_PROMPT: str = """You are an expert in Lean 4 programming and first-order logic theorem proving. Your role is to provide pedagogical guidance by analyzing proof attempts and their errors to create structured learning experiences. Reflect on past failures, noting sub-propositions and error messages, and use these insights to inform your analysis. 
...
{error_record}
[[ANALYSIS]]
Analyze in <= 100 words: analyze the proof attempts and provide structured insights following the output format.
"""

def build_messages(data, use_system_prompt = True, backtrace=10):
    ...
    for d in data:
        ...
        labeled_error_list = labeled_error_list[-backtrace:]
        labeled_error_record = '\n'.join(labeled_error_list)
        
        # if len(labeled_error_list) != 0:
        if use_system_prompt:
            ...
            messages.append([
                    {
                        'role': 'user', 
                        'content': f"{SUBGOAL_ERROR_ANALYZE_SYSTEM_PROMPT}\n{SUBGOAL_ERROR_ANALYZE_USER_PROMPT.format(context=context, error_record=labeled_error_record)}"
                    }
                ]
            )
...
async def process_tasks(input_path):
    ...
    messages = build_messages(dataset, backtrace=backtrace, use_system_prompt=config['use_system_prompt']) 
