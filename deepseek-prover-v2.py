...
DIRECT_PROVE_SYSTEM_PROMPT: str = """
"""
...
Exclude the context from the final proof.
"""

#############未适应deepseek
ERROR_FB_PROVE_SYSTEM_PROMPT: str = """Generate a proof for the provided conjecture, taking into account the previous failed attempt. 
...
ERROR_FB_PROVE_USER_PROMPT: str = """
[[CONTEXT]] {context}
[[CONJECTURE]] {conjecture}
[[PREVIOUS ATTEMPT]]
[ERROR-PROOF] {error_proof}
[ERROR-MESSAGE] {error_msg}
[[CORRECT-PROOF]]
"""


def build_messages(data, use_error=False, use_system_prompt = True):
    ...
    for d in data:
        ...
        for solution in reversed(d['solutions']):
            if solution['code']:
                error_example = solution['code']
                err_msg = solution['error_msg']
                break  
        
        if use_error and error_example:
            
            if use_system_prompt:
                messages.append([
                            {'role': 'system', 'content': ERROR_FB_PROVE_SYSTEM_PROMPT},
                            {'role': 'user', 'content': ERROR_FB_PROVE_USER_PROMPT.format(
                                context=context,
                                conjecture = d['conjecture'], 
                                error_proof=error_example, 
                                error_msg=err_msg)}
                ])
                ...
                messages.append([
                        {
                            'role': 'user', 
                            'content': f"{ERROR_FB_PROVE_SYSTEM_PROMPT}\n{ERROR_FB_PROVE_USER_PROMPT.format(context=context,conjecture = d['conjecture'], error_proof=error_example, error_msg=err_msg)}"
                        }
                    ]
                )
            ...
            if use_system_prompt:
                messages.append([
                            ...
                            {'role': 'user', 'content': DIRECT_PROVE_USER_PROMPT.format(context=context,conjecture = d['conjecture'])}
                ])
                
            else:
                messages.append([
                        {'role': 'user', 'content': f"{DIRECT_PROVE_SYSTEM_PROMPT}\n{DIRECT_PROVE_USER_PROMPT.format(context=context, conjecture = d['conjecture'])}"}
                    ]
                )
...
def extract_lean_code(text):
    ...
    if matches:
        return matches[-1].strip()  # 返回最后一个匹配的内容

    pattern2 = r"```lean(.*?)```"
    matches = re.findall(pattern2, text, re.DOTALL)

    return matches[-1].strip() if matches else False
...
async def process_tasks(input_path):
    ...
    if os.path.exists(output_file):
        ...
        with open(output_file, 'r') as f3:
            ...
            false_results = [r for r in results if r.get('score', False) == False and r.get('time', 0) < 10]
            
            if len(false_results) != 0:
                time = false_results[0].get('time', 0) + 1
            else:
                time = 11
            
                
            results  = [d for d in results if d.get('score', False) == True or d.get('time', 0) ==10] 
    ...
    for i, response_message_batch in enumerate(response_message_batches):
        ...
        for j, response in enumerate(response_message_batch):
            ...
            if dataset_index >= len(dataset) or not response or response[0] == '':
                continue
